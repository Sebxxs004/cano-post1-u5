...
fun Throwable.toAppError(): AppError = when (this) {
    is IOException -> AppError.Network(message ?: "Sin conexion")
...
fun AppError.toMessage(): String = when (this) {
    is AppError.Network -> "Sin conexion a internet. Verifica la red."
    is AppError.Unauthorized -> "Sesion no autorizada ($code)."
    is AppError.NotFound -> "$resource no encontrado."
    is AppError.Server -> "Error del servidor ($code). Intenta mas tarde."
